...
def impellerIn(Qi,rot,Tatm,uvRatio):
    ...
    Q1 = 0
    iq = 0
    print("- The velocity ratio U/V = {}".format(uvRatio))
    while (Q1 == 0) :
        Q1 = Qi
...
def design(nameF,Qi,Pin,DPi,Nb,th,w,T,c1,c2,uvRatio):
    ...
    while corr == False:
        ...
        Qi = Q1 #corrected value of the flow aret due to the compression effect
        rot = rot1
        ##Stimare di nuovo la velocita' e la geometria
    ...
    Ainlet =  [D1,Dduct,Lduct,U1,V1,Vduct,W1,beta1,B1,A1,Aduct,c1,uvRatio]
...
def inPutParam():
    ...
    alfAtt = float(input("- Select the attack angle (35-45 deg):  ")) # Austin Church - U/V = 1.1
    ...
    return nameF,qo,pin,dPo,nb,th,w,T,c1,c2,uvRatio

##############################################################################
###   Main    #####
#Design input parameter
nameF,Qo,Pin,DPo,Nb,th,w,T,c1,c2,uvRatio = inPutParam()
Qi = Qo
DPi = DPo
#Design Function
A = design(nameF, Qi, Pin, DPi, Nb, th, w, T,c1,c2,uvRatio)
#Design output
D1,Dduct,Lduct,U1,V1,Vduct,W1,beta1,B1,A1,Aduct,c1,uvRatio = A[0]
...
#A[0] = [D1,Dduct,Lduct,U1,V1,Vduct,W1,beta1,B1,A1,Aduct,c1,uvRatio]
#A[1] = [U2,D2,V2,V2m,W2,alpha2,beta2,B2,A2,Pow,Ws,slipf,c2]
